        ...
        parts.each do |part|
          if part.multipart?
            part.parts.each do |part2|
              result = part2.unquoted_body if part2.content_type =~ /(html|text)/i
            end
          elsif !attachment?(part)
            result = part.unquoted_body if part.content_type =~ /(html|text)/i
          end
        end
      else
        result = unquoted_body if content_type =~ /(html|text)/i
      end
      result
    end
